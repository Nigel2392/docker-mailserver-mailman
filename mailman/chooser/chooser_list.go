...
func (v *ChooserListPage[T]) getList(req *http.Request, results []T) any {
	...
	var listObject = list.NewListWithGroups(
		req,
		attrs.NewObject[T](v._Definition.Model),
		results,
		v.GetListColumns(req),
		func(r *http.Request, obj T, cols []list.ListColumn[T]) list.ColumnGroup[T] {
			return &wrappedColumnGroup[T]{
				ListColumnGroup: list.NewColumnGroup(r, obj, cols),
				_Definition:     v._Definition,
			}
